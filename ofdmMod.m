function [signal_LED_time_serial_output, BitSource] = ofdmMod(Num_frame, N_CP, M_QAM, Num_LED, N_FFT, range, h_actual)
%     K_QAM = log2(M_QAM);
%     N_FFT_half = N_FFT/2;
    numBit = N_FFT;%252000
...
%         Energy_M_QAM = 6;
%     else
%         if sqrt(M_QAM) == fix(sqrt(M_QAM))% fix х║об╫Г
...
%     end
%     
%     BitSource = randi([0 , 1] , 1 , numBit);  % source bit %252000
    BitSource = ones(Num_frame , numBit); 
%     BitSource_PPM = zeros(1 , 2 * numBit);     % source bit after PPM %504000
%     for jj=1:numBit;
%         if(BitSource(jj)==0)
...
%     symbol_decimal = bi2de(reshape(BitSource , numel(BitSource)/K_QAM , K_QAM) , 'left-msb');% 126000*1
%     symbol_input = reshape(symbol_decimal, [], Num_frame)';% 2000*63 
%     symbol_MQAM_matrix = 1 / sqrt(Energy_M_QAM) * qammod(symbol_input, M_QAM);% 2000*63
...
%         sig_ofdm_freq{i_LED}(: , range(i_LED, 3) : range(i_LED, 4)) = conj(sig_ofdm_freq{i_LED}(:, range(i_LED, 2):-1:range(i_LED, 1))); 
        ...
        signal_LED_time_no_CP{i_LED} = ifft(BitSource, N_FFT, 2);
